...
import { Utensils, Coffee, Landmark, ShoppingBag, AlertTriangle } from "lucide-react";
    ...
    <div>
      ...
      {poiOffline && (
        <div style={{ display: "flex", alignItems: "center", gap: 6, background: T.amberDim, border: `1px solid rgba(226,136,61,0.35)`, borderRadius: 10, padding: "8px 12px", marginBottom: 10, fontSize: 12 }}>
          ...
          Canlı yer servisine şu an ulaşılamadı, {trip.city} için önceden hazırlanmış günlük gösteriliyor.
        </div>
      )}
      {poiError && <Empty text={`Bu şehir için canlı veya örnek veri bulunamadı (${poiError}).`} />}

      {groups.map(g => {
        const entries = poiEntries(poi?.[g.key], g.key);
        return (
          <div key={g.key}>
            ...
            {entries.length === 0 ? (
              ...
              <>
                <div style={{ fontFamily: "'Fraunces',serif", fontStyle: "italic", fontSize: 13, color: T.muted, marginBottom: 8 }}>{VLOG_INTRO[g.key]}</div>
                {entries.map((item, i) => (
                  <div key={i} style={{ display: "flex", gap: 10, background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, padding: 10, marginBottom: 8 }}>
                    <div style={{
